...
def scale_free_graphX(taskID, nodeID, NORnodes, Nusers,
                     ...
                     create_using=None,
                     seed=None):
    """Return a scale free directed graph. Also return a list of Neo4j Query commands to add the needed tasks and ORpair nodes added to the graph G.
...
def scale_free_graphXL(NORnodes, nU,
                         alpha=0.15,
                         beta=0.70,
                         gamma=0.15,
                         directed_p = 0.05,
                         ...
                         create_using=None,
                         seed=None):
    """Return a scale free directed graph. Unlike scale_free_graphX, return lists of the updates grouped by each ORpair to add.

    Parameters
    ----------
    n : integer
        Number of nodes in graph
    nU : Every nU ORpairs add anew user
    alpha : float
        Probability for adding a new node connected to an existing node
        chosen randomly according to the in-degree distribution.
    beta : float
    ...
    nodeID = 3; taskID = 3; Nusers = 3;
    delta = (delta_in + delta_out) / 2
    users = npr.randint(1, 3+1, size=3)
    if create_using is None:
        # start with 3-cycle
        G = nx.MultiDiGraph()
        G.add_edge('1','2',key='1',user=str(users[0]))
        G.add_edge('2','3',key='2',user=str(users[1]))
        G.add_edge('3','1',key='3',user=str(users[2]))
        # First add the initial users and tasks
        for w in range(1,3+1):
            updates.append( (add_user(w), add_task(w), '') )
        updates.append( ('', '', (1, add_ORnode(1, (1, 2, users[0])) )) )# add to neo4j
        updates.append( ('', '', (2, add_ORnode(2, (2, 3, users[1])) )) )# add to neo4j
        updates.append( ('', '', (3, add_ORnode(3, (3, 1, users[2])) )) )# add to neo4j
    else:
        # keep existing graph structure?
        G = create_using
        if not (G.is_directed() and G.is_multigraph()):
    ...
    # seed random number generated (uses '' as default)
    random.seed(seed)
    #z = {'in':0,'out':0,'mix':0}
    while nodeID < NORnodes:
        update = ('', '', '')

        # Add new user every nU ORpairs
        if not nodeID % nU:
            Nusers += 1;
            update = (add_user(Nusers), '', '')
        userID = npr.randint(Nusers) + 1
        ...
        if r<alpha:
            ...
            # add new node v
            taskID += 1; v = taskID;
            update = (update[0], add_task(taskID), '')
            # choose w according to in-degree and delta_in
            if rd < directed_p:
                w = _choose_node(G, G.in_degree(),delta_in)
            else:
        ...
        nodeID +=1
        # add to networkX
        G.add_edge(str(v), str(w), key = str(nodeID), user = str(userID))
        # add to neo4j update list
        updates.append( (update[0], update[1], (nodeID, add_ORnode(nodeID, (v, w, userID))) ) )
        #s = (nodeID, (v, w, userID))
        #print("%s %s" % s)
    #print("Chosen by (in, out, mix):: (%s, %s, %s)" % (z['in'], z['out'], z['mix']))

    return G, updates, Nusers, taskID
